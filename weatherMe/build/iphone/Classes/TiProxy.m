...
//Rare exceptions to indicate a bug in the _weatherme code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_weatherme = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_weatherme]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.weatherme.module"
