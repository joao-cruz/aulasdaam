function Controller() {
    function lookFor() {
        ...
        var getWeatherURLForCity = "http://api.openweathermap.org/data/2.5/weather?q=" + $.cityName.value + "&units=metric";
    ...
    openFavourites ? $.__views.__alloyId11.addEventListener("click", openFavourites) : __defers["$.__views.__alloyId11!click!openFavourites"] = true;
    ...
    lookFor ? $.__views.__alloyId18.addEventListener("click", lookFor) : __defers["$.__views.__alloyId18!click!lookFor"] = true;
        ...
        if (Ti.Geolocation.locationServicesEnabled) {
            ...
            Titanium.Geolocation.getCurrentPosition(function(e) {
                if (e.error) Ti.API.error("Error: " + e.error); else {
                    ...
                    var getWeatherURLForCoordinates = "http://api.openweathermap.org/data/2.5/weather?lat=" + latt + "&lon=" + longt + "&units=metric";
                    var xhr = Titanium.Network.createHTTPClient({
                        onload: function() {
                            var result = JSON.parse(this.responseText);
                            $.mapview.removeAllAnnotations();
    ...
    $.weatherWindow.open();
    __defers["$.__views.__alloyId11!click!openFavourites"] && $.__views.__alloyId11.addEventListener("click", openFavourites);
    __defers["$.__views.__alloyId18!click!lookFor"] && $.__views.__alloyId18.addEventListener("click", lookFor);
    _.extend($, exports);
}
