...
//Rare exceptions to indicate a bug in the _dreamdestinationaday code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_dreamdestinationaday = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_dreamdestinationaday]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.dreamdestinationaday.module"
