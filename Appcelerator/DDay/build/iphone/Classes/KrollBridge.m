...
NSString * DreamDestinationADay$ModuleRequireFormat = @"(function(exports){"
...
@implementation DreamDestinationADayObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_dreamdestinationaday);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create DreamDestinationADay global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "DreamDestinationADay" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_dreamdestinationaday = [[DreamDestinationADayObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_dreamdestinationaday];
	
	NSString *_dreamdestinationadayNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _dreamdestinationadayNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_dreamdestinationaday valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:DreamDestinationADay$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.dreamdestinationaday.kroll" 
